...
const LedgerManager = {
  ...
  checkAndMigrateOldData() {
    ...
    if (ledgers.length === 0) {
      const oldTransactions = localStorage.getItem("expense_tracker_transactions");
      if (oldTransactions) {
        // Ask user if they want to migrate
        const confirmMigrate = confirm("Found existing transaction data. Would you like to migrate it to a new ledger?");
        
        if (confirmMigrate) {
          const ledgerName = prompt("Enter a name for your ledger:", "My Ledger");
  ...
  updateLedgerSelector() {
    ...
    const currentLedgerDisplay = document.getElementById("current-ledger-display");
  ...
  setupEventListeners() {
    ...
    selector.addEventListener("change", (e) => {
      ...
      if (selectedLedger) {
        ...
        document.getElementById("current-ledger-display").textContent = `Current Ledger: ${selectedLedger}`;
    ...
    document.getElementById("new-ledger-btn").addEventListener("click", () => {
      const name = prompt("Enter a name for the new ledger:", `Ledger ${new Date().toLocaleDateString()}`);
      
      if (name) {
        // Check if ledger name already exists
        const ledgers = TransactionManager.getLedgers();
        if (ledgers.includes(name)) {
          alert(`A ledger named "${name}" already exists. Please choose a different name.`);
          return;
        }
    ...
    document.getElementById("delete-ledger-btn").addEventListener("click", () => {
      const activeLedger = TransactionManager.getActiveLedger();
      
      if (activeLedger) {
        const confirmDelete = confirm(`Are you sure you want to delete the ledger "${activeLedger}"? This action cannot be undone.`);
